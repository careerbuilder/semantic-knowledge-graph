...
public class FallbackScorerTest
{

    QueryRunner[] fgRunners = new QueryRunner[4];
    QueryRunner[] replacements = new QueryRunner[2];

    ResponseNode response = new ResponseNode();
    RequestNode request = new RequestNode();
    List<QueryRunner> runners = new LinkedList<>();
    List<QueryRunner> replacementRunners = new LinkedList<>();
    @Mocked
    DocSet fgDomain;
    @Mocked
    DocSet bgDomain;
    ...
    public void init()
    {
        ...
        fgRunners[2] = new QueryRunner(null, null, null, QueryRunner.QueryType.FG, 2);
        fgRunners[3] = new QueryRunner(null, null, null, QueryRunner.QueryType.FG, 3);
        fgRunners[0].result = 0;
        fgRunners[1].result = 3;
        fgRunners[2].result = 1;
        fgRunners[3].result = 3;

        replacements[0] = new QueryRunner(null, null, null, QueryRunner.QueryType.FG, 0);
        replacements[1] = new QueryRunner(null, null, null, QueryRunner.QueryType.FG, 2);
    ...
    {
        FallbackScorer target = new FallbackScorer();
    ...
    {
        FallbackScorer target = new FallbackScorer();
        List<QueryRunner> original = new LinkedList<>(runners);
